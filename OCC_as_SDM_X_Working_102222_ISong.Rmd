...
p_load(tmap, virtualspecies, SDMtune, precrec, Metrics, prg, raster, rasterVis, sf, tidyverse)
...
## Formulas
forms.gaus.prime <- function(Xs){ # prime
  # I didn't understand why Xs[i] is used.
  ((1/(sqrt(2*pi)*(Xpop*apop.prime + sigma.dprime)))*exp((-0.5)*((Xs[i]-mu)^2)/((Xpop*apop.prime + sigma.dprime)^2))) -> vc
  # ((1/sqrt(2*pi)*(Xpop*apop.prime + sigma.dprime))*exp(1)^(-(Xs[i]-mu)^2/2*((Xpop*apop.prime + sigma.dprime))^2)) -> vc
  # What is your justification of setting thresholds 0 and 1?
  idx.over <- vc > 1
  idx.lower <- vc < 0
  vc[idx.over] <- 1
  vc[idx.lower] <- 0
...
# seems like it does not assume that there is no "prime" variable
forms.gaus <- function(Xs){
  ((1/(sqrt(2*pi)*sigma))*exp(-((Xs[i]-mu)^2)/(2*(sigma^2)))) -> vc
  # don't spare parentheses -- try comparing (4-2)^2/2*3^2 and ((4-2)^2)/(2*(3^2))
  # ((1/sqrt(2*pi)*sigma)*exp(1)^(-(Xs[i]-mu)^2/2*(sigma)^2)) -> vc
  idx.over <- vc > 1
  idx.lower <- vc < 0
  vc[idx.over] <- 1
...
forms.linear.prime <- function(Xs){ # prime
  ((Xpop*apop.prime + apop.prime)*Xs[i] + b) -> vc
  idx.over <- vc > 1
  idx.lower <- vc < 0
  vc[idx.over] <- 1
  vc[idx.lower] <- 0
...
## Re-test with corrected formulae
...
## See Here: Chunk 14
```{r Testing foreach to raster level-functional chunk 14}
Xpop <- runif(100000*nlayers(Xs.choosen), min = 0, max = nlayers(Xs.choosen))
acc.lim <- rev(seq(from = 0.000001, to = 0.0001, by = 0.0000005))
##############
## FOR EACH ##
##############

exlist = c('Xs', 'e.variable.name.range', 'e.variable.n.range', 'Xpop')
vcf = vc.filled[1:100]

foreach(seed = c(1,1000), .packages = c("magrittr", "purrr", "raster"),
        .export = exlist, .combine = 'rbind') %:% 
  foreach(i = vc.filled, .combine = 'c') %dopar% {
    set.seed(seed)
    ## parameter sampling
    mu <- sample(mu.range, 1)
    sigma <- sample(sigma.range, 1)
    ...
    map2(
      sample(forms, nlayers(Xs.choosen), replace = TRUE), # which formula to apply
      map(as.list(Xs.choosen), function(x){list(Xs = x)}),  # supply choosen variables as list
      function(fn, args){exec(fn, !!!args)}
      ) -> test
    as.data.frame(test) %>% 
      rowSums() -> test # formula result 우변, 좌변은 Xpop
    ...
    mean(Xpop[inequation])
  } -> r.vector
r.vector.test = r.vector
...
```{r}
r.vector = readRDS("OCC_sim_vector.rds")
# once r.vector.test is caculated.

raster.blank = Xs[[1]]
raster::values(raster.blank)[!is.na(raster::values(raster.blank))] <- r.vector[1,]
raster.blank
plot(raster.blank)
