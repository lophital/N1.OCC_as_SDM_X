...
virtualspecies::generateRandomSp(raster.stack = stack(Xs[[1]]),
                                 approach = "response",
                                 rescale = TRUE, # normalization Y
                                 rescale.each.response = TRUE, # normalization X 
                                 convert.to.PA = FALSE,
...
#                  rescale.each.response = FALSE, # rescale X
#                  #formula = "layer.1 + layer.2", #편반응결합에 쓰인 공식 
...
vs.rnd$details$parameters$layer.1$args[[1]] -> a1
vs.rnd$details$parameters$layer.1$args[[2]] -> b01
vs.rnd$details$parameters$layer.2$args[[1]] -> a02
vs.rnd$details$parameters$layer.2$args[[2]] -> b02

vs.test <- rep(list(vs.rnd), length(apop))
for(i in 1:length(apop)){
  vs.test[[i]]$suitab.raster <- (1/(1-apop[i]*Xs[[1]]))*((a1*Xs[[1]])+(a02*Xs[[2]]) + b01 + b02)
}
