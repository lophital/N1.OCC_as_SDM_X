...
set.seed(1)
stack(
NLMR::nlm_gaussianfield(10, 10),
NLMR::nlm_gaussianfield(10, 10),
NLMR::nlm_gaussianfield(10, 10)) -> Xs
plot(Xs)
```
...
results[[seed]] <- tribble(~seed, ~vc, ~param,
        seed, r.vector.test.foreach[[seed]], c(mu, sigma, sigma.dprime, a, apop.prime, apop.dprime, b))
}
...
raster.fill[raster::values(raster.fill)] <- r.vector.test.foreach[[2]]
plot(raster.fill)



raster.Xpop <- my.rescale(raster.fill) # rescale to 0~1
plot(raster.Xpop)
```
...
r.vector[!is.na(r.vector)] -> r.vector
...
```{r}
# once r.vector.test is caculated.
raster.fill[raster::values(raster.fill)] <- r.vector.test.foreach
plot(raster.fill)
...
### test
acc.lim <- rev(seq(from = 0.0000000001, to = 0.0001, by = 0.000000005))
acc.lim <- rev(seq(from = 0.00000005, to = 0.00005, by = 0.00000005))
r.vector <- vector()
inequation <- abs(((1/sqrt(2*pi)*(Xpop*apop_1+sigma_1))*exp(1)^(-(Xs[[1]][i]-mu_01)^2/2*((Xpop*apop_1+sigma_1))^2)) +
              ((1/sqrt(2*pi)*sigma_02)*exp(1)^(-(Xs[[2]][i]-mu_02)^2/2*(sigma_02)^2)) - Xpop) < 0.1
