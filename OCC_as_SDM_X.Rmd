...
forms.gaus.prime <- function(Xs){ # prime
  ((1/sqrt(2*pi)*(Xpop*apop.prime + sigma.dprime))*exp(1)^(-(Xs[i]-mu)^2/2*((Xpop*apop.prime + sigma.dprime))^2))
}


forms.gaus <- function(Xs){
  ((1/sqrt(2*pi)*sigma)*exp(1)^(-(Xs[i]-mu)^2/2*(sigma)^2))
}

forms.linear.prime <- function(Xs){ # prime
  ((Xpop*apop.prime + apop.dprime)*Xs[i] + b)
}

#ifelse(cars$cyl > 4, 1, 0)

forms.linear <- function(Xs){
  ifelse(test = (a *Xs[i] + b) > 1, 
         yes = signif(a *Xs[i] + b, 1), #땜빵. MAYBE could reach over 1, then fix this. 
         no = (a *Xs[i] + b)
  )
}
...
## MC equation
inequation <- abs((forms.gaus.prime(Xs[[1]])) + forms.gaus(Xs[[2]]) - Xpop) < 0.1
...
forms.gaus.prime(Xs[[1]]) %>% my.rescale.vc()
